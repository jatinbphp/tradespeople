    ...
    <div class="row">
              ...
              ?>
                  <li class="user_list_<?=$serialNumber;?>">
                    <a href="javascript:void(0);" onclick="get_message(<?=$chatUser['admin_chat_id'];?>, <?=$chatUser['id'];?>, '<?=$chatUser['f_name'] .' ' .$chatUser['l_name']?>', '<?=$chatUser['email'];?>', 'user_list_<?=$serialNumber;?>');">
                      ...
                      <p class="namechat"><b><?=$chatUser['f_name'] .' ' .$chatUser['l_name'];?></b> <?php if($chatUser['type'] == 2){echo 'Homeowner'; }elseif($chatUser['type'] == 1){ echo 'Tradesman'; }else{ echo 'Affiliate'; }?> </p>
                      <?php
                      ...
                      ?>
                          ...
                          ?>
                          <span class="text_usch">Ticket ID: <?=$chatUser['ticket_id'];?></span>
                      <?php
                        }
                      ?>
                      <?php if($chatUser['ticket_status']==1){ ?> 
      ...
      <div class="col-sm-8" id="messages_section">
        <div class="right_messge show-mmm">
          ...
          <div id="rep_btn">
            <a class="btn btn-danger pull-right" onclick="close_ticket();">Close</a>
          ...
          <!-- <div class="messge_send">
            <div class="input-group">
              ...
              <span class="input-group-btn">
                ...
                <button class="btn btn-danger btn-lg" id="close_ticket_btn" onclick="close_ticket();" >Close</button>
              </span>
            </div>
          </div> -->
        </div>
    ...
    <div class="modal-content">
      <form method="post" onsubmit="send_message(event);" id="messageform" name="messageform" >
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" id="message_send_btn" onclick="send_message();" >Send</button>
        </div>
      </form>
...
<script>
  function close_ticket() {
    if(confirm('Are you sure want to closed chat?')){
      var admin_chat_id = $('#admin_chat_id').val();
      var user_id = $('#receiver_id').val();
      $.ajax({
        type:'post',
        url: site_url + 'Admin/Chats/close_ticket',
        data:{
  ...
  function get_message(admin_chat_id, user_id, username, email, activeList){
    ...
    $.ajax({
      ...
      success:function(response){
        ...
        if(response.ticket_status==1){
          $('#rep_btn').css('display', 'none');
        }else{
          $('#rep_btn').css('display', 'block');
        }
