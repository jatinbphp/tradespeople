								...
								<tbody>
									<?php  $i = 1; foreach($listing as $data){   ?>
										<tr>
											...
											<td>
												<?php 
													$main_category = $this->Common_model->GetSingleData('service_category',array('cat_id'=>$data['cat_parent']));
											...
											<td>
												<a href="<?= base_url('category/').$data['slug']?>" target="_blank" style="font-size:16px" class="btn btn-warning btn-xs">View</i></a>
												...
												<a style="font-size:16px" onclick="return  confirm('Are you sure want to delete this Category?');" href="<?php echo base_url();?>Admin/Location_Local_Category/delete_category/<?php echo $data['cat_id'];?>?redirect_location=<?= ($locations) ? $locations['id'] : ''?>" tyle="float: right;" class="btn btn-danger btn-xs">Delete</a>
												<?php if($data['enabled']) {?>
													<a style="font-size:16px" onclick="return  confirm('Are you sure want to disable this Category?');" href="<?php echo base_url();?>Admin/Location_Local_Category/disable_category/<?php echo $data['cat_id'];?>?redirect_location=<?= ($locations) ? $locations['id'] : ''?>" tyle="float: right;" class="btn btn-warning btn-xs">Disable</a>
												<?php } else {?>  
													<a style="font-size:16px" onclick="return  confirm('Are you sure want to enable this Category?');" href="<?php echo base_url();?>Admin/Location_Local_Category/enable_category/<?php echo $data['cat_id'];?>?redirect_location=<?= ($locations) ? $locations['id'] : ''?>" tyle="float: right;" class="btn btn-success btn-xs">Enable</a>
												<?php } ?>
											</td>
										</tr>
										<!-- Modal -->
										<div class="modal fade slug-modal" id="editmyModal<?php echo $data['cat_id'];?>" role="dialog">
											<div class="modal-dialog">
												...
												<div class="modal-content">
													...
													<form action="<?php echo base_url(); ?>Admin/Location_Local_Category/edit_category?redirect_location=<?= ($locations) ? $locations['id'] : ''?>" method="post" enctype="multipart/form-data" autocomplete="off">
...
<div class="modal fade slug-modal" id="myModal" role="dialog">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<form action="<?php echo base_url(); ?>Admin/Location_Local_Category/add_category?redirect_location=<?= ($locations) ? $locations['id'] : ''?>" method="post" enctype="multipart/form-data" autocomplete="off">
				<div class="modal-body form_width100">
					<div class="form-group">
						<label for="email">Select category:</label>							
...
<script>
	$(document).ready(function(){
        ...
        // Load data from an Ajax source
			"ajax": {
				"url": site_url+"Admin/Location_Local_Category/getLocalCategory",
				"type": "POST"
			},
