			...
			<div class="<?php echo $col_sm_8; ?>">
				...
				<div class="dashboard-white dashboard-white2">
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="ppprogg">
								<div class="proosimg_us">
									<img src="img/icon_loaddew.png">
								</div>
					...
					<div class="row">
						<div class="col-sm-5">
							<div class="img-name1">
								...
								<div class="names1">
									<h4><b><a onclick="show_tradesment_profile(<?php echo $get_users['id']; ?>);" href="javascript:void(0);"><?php echo $get_users['trading_name']; ?></a></b><br><a class="small" onclick="show_tradesment_profile(<?php echo $get_users['id']; ?>);" href="javascript:void(0);">View profile</a></h4>
						...
						<div class="col-sm-7">
							<div class="text-right">
								<p><b><i class="fa fa-gbp"></i><?php echo $a['bid_amount']; ?> GBP</b> in <?php echo $a['delivery_days']; ?> day(s)</p>
									...
									<?php
										if($this->session->userdata('user_id')==$a['bid_by']){
											if($setting[0]['payment_method'] == 1){
												if($a['status']==1 || $a['status']==3 || $a['status']==5 || $a['status']==7 || $a['status']==10) {
													...
													if($a['hiring_type']==0 ||	($get_plan_bids && $get_plan_bids['up_status']==1 && strtotime($get_plan_bids['up_enddate'])>=strtotime(date('Y-m-d')) ||
													$get_plan_bids['valid_type']==1) || $get_chat_paid){
									?>
...
<?php } ?>
										...
										<?php } else { ?>
										 <a  class="btn btn-danger" href="<?php echo site_url('posts/reject_award/'.$a['id'].'/'.$a['job_id']); ?>" onclick="return confirm('Are you sure you want to reject this offer?');">Reject</a>
										<?php } ?>
...
<!-- instant paymet-->
        ...
        <div class="dashboard-white edit-pro89"> 
			...
			<div class="col-sm-4">
				...
				<?php 
						$get_jobs=$this->common_model->get_single_data('tbl_jobs',array('job_id'=>$_REQUEST['post_id'])); ?>
					<?php if($get_jobs['budget'] && $get_jobs['budget'] > 0){ ?>
					<p><b>Budget</b></p>
					<p>
						
						£<?php echo $get_jobs['budget'];  ?><?php echo ($get_jobs['budget2'])?' - £'.$get_jobs['budget2']:''; ?>
					</p>
					<?php } ?>
					<p><b>Quotes</b></p>
