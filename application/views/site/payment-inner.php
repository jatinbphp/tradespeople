		...
		<div class="row">
			<div class="col-sm-9">
              ...
              <h3><i class="fa fa-gbp"></i><?php echo $get_bid_post[0]['bid_amount']; ?> GBP</h3>
						...
						<div class="names1">
							<h4><b><a onclick="show_tradesment_profile(<?php echo $get_users['id']; ?>);" href="javascript:void(0);"><?php echo $get_users['trading_name']; ?></b></a></h4>
                ...
                <?php } ?>
								...
								</span> (<?php if($get_users['total_reviews']){ echo $get_users['total_reviews']; }else{ echo "0"; } ?> reviews)
					...
					<div class="Required_mne">
						...
						<?php if($this->session->userdata('type')==2){ ?>
							...
							<?php //if($get_bid_post[0]['status']==7) { 
							
							$active_milestones = $this->common_model->get_active_milestone($get_bid_post[0]['id']);
							
							if($active_milestones['amount'] < $get_bid_post[0]['bid_amount']){ ?>
							...
							<?php /*} else { ?>
								...
								<button disabled onclick="swal('You can not Create Milestone before job accept by tradesmen!');" class="btn btn-primary pull-right" style="color: #fff"; >Create Milestone</button>
						...
						<?php }else{ ?>
							...
							<?php if($get_bid_post[0]['paid_total_miles'] < $get_bid_post[0]['bid_amount']){ ?>
								...
								<button disabled onclick="swal('You can not request for milestone before job accept by you!');" class="btn btn-primary pull-right" style="color: #fff"; >Request Milestone</button>
								...
								<div class="col-sm-4">
									<div class="from-group">
										...
										<div class="input-group">
											...
											<input type="number" class="form-control miamount_1" placeholder="Project Amount" min="1" value="<?php echo ($remaining)?$remaining:'';?>" required name="tsm_amount1">
										</div>
									</div>
								</div>
							</div>
				...
				<div class="modal-body">
					...
					<fieldset>
						...
						<div class="common_pay_main_div text-center instant-payment-button" style="display:none;">
							<div class="alert alert-danger">Insufficient amount in your wallet. Click on pay now and add money to wallet.</div> <br>
							...
							<div class="card pay_btns  all-pay-tooltip">
								<div class="col-sm-4" style="padding: 0;">
									...
									<div onclick="show_lates_stripe_popup(<?php echo $amount_wp2; ?>,<?php echo $get_commision[0]['p_min_d']; ?>,7);" class="pay_btn strip_btn" id="strip_btn"><img src="<?= base_url(); ?>img/pay_with.png"></div>  <?= $strip_Pay_info; ?>
								</div>
								</div>
								<div class="col-sm-4"  style="padding: 0;">
									<div class="bonnnttlllo4">
						...
						<table class="table">
							...
							<tbody>
								<tr>
									<td>
									<?php 
										$yrdata= strtotime($get_bid_post[0]['awarded_date']);   
										echo date('d', $yrdata)." ".date('F', $yrdata); ?> 
										<?php echo date('Y',$yrdata); ?>
									</td>
									<td>Project Budget</td>
									<td>
										<?php if($get_bid_post[0]['bid_amount']==$get_bid_post[0]['paid_total_miles']){ ?>
										Completed
										<?php }else{ ?>
										Pending
										<?php } ?>
									</td>
									<td><i class="fa fa-gbp"></i><?php echo $get_bid_post[0]['bid_amount']; ?> GBP</td>
...
<div class="modal fade popup" id="accept_releaseMS<?php echo $mile['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			...
			<form method="post" id="accept_releaseMSForm<?php echo $mile['id']; ?>" onsubmit="return accept_requested_milestone(<?php echo $mile['id']; ?>)">
              ...
              <div class="card_ pay_btns_<?php echo $mile['id']; ?> all-pay-tooltip">
                <div class="col-sm-4" style="padding: 0;">
                  <div data-id="<?php echo $mile['id'];?>" onclick="show_lates_stripe_popup(<?php echo $amount_wp2; ?>,<?php echo $get_commision[0]['p_min_d']; ?>,8,<?php echo $mile['id']; ?>);" class="pay_btn_<?php echo $mile['id']; ?> strip_btn" id="strip_btn_<?php echo $mile['id']; ?>"><img src="<?= base_url(); ?>img/pay_with.png"></div>  <?= $strip_Pay_info; ?>
                </div>
                <div class="col-sm-4" style="padding: 0;">
                  <div class="pay_btn_<?php echo $mile['id']; ?> paypal_btn" id="paypal_btn_<?php echo $mile['id']; ?>"></div>  <?= $paypal_Pay_info; ?>
...
<ul class="dropdown-menu" style="text-align: left;">
	...
	<li><a href="<?php echo site_url('dispute/'.$mile['id'].'/'.$mile['post_id']); ?>">View Dispute</a></li> 
	
	<?php
	if($mile['status']==5){
		
	$check_disput_user = $this->common_model->GetColumnName('tbl_dispute',array('mile_id'=>$mile['id'],'disputed_by'=>$user_id2),array('ds_id'));
	if($check_disput_user){ ?>
	<li><a onclick="return confirm('Are are sure, you want to cancel this dispute?');" href="<?php echo site_url().'cancel_dispute/'.$check_disput_user['ds_id'].'/'.$_REQUEST['post_id']; ?>">Cancel Dispute</a></li> 
	<?php } ?>
...
<div class="modal fade popup" id="dispute_mileds<?php echo $mile['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			...
			<div id="msg1"><?= $this->session->flashdata('msg'); ?></div>
			<form method="post" id="dispute_milesss<?php echo $mile['id']; ?>" enctype="multipart/form-data" action="<?php echo site_url('dispute-job/'.$mile['id']); ?>" onsubmit="return edit_portfolio();">
				<div class="modal-body">
					<fieldset>
						<div id="milesssstone">
							<div class="row">
								<div class="col-sm-12">
									<div class="from-group">
										<label class="control-label" for="textinput"><b>Reason of Dispute</b></label>  
										<textarea class="form-control textarea" name="dispute_reason" rows="5" required></textarea>
										<br>
										...
										<input type="checkbox" name="milestones[]" <?php if($mile['id']==$m['id']){ ?>checked<?php } ?> value="<?php echo $m['id']; ?>"> <?php echo $m['milestone_name']; ?><br>
										<?php } ?>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
					<button type="submit" class="btn btn-warning dispute_btn">Submit</button>
				</div>
			</form>
		</div>
	</div>
										...
										<a href="<?php echo site_url('dispute/'.$mile['id'].'/'.$mile['post_id']); ?>" class="btn btn-primary btn-sm">View Dispute</a>
										<?php 
	...
	if($check_disput_user){ 
	?>
	<a onclick="return confirm('Are are sure, you want to cancel this dispute');" href="<?php echo site_url().'cancel_dispute/'.$check_disput_user['ds_id'].'/'.$_REQUEST['post_id']; ?>" class="btn btn-primary btn-sm">Cancel Dispute</a> 
	<?php } ?>
...
<ul class="dropdown-menu" style="text-align: left;">
	...
	<li><a href="#" data-target="#dispute_mile<?php echo $mile['id']; ?>" data-toggle="modal">Dispute</a></li>
...
<div class="modal fade popup" id="dispute_mile<?php echo $mile['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			...
			<form method="post" id="dispute_miles<?php echo $mile['id']; ?>" enctype="multipart/form-data" action="<?php echo site_url('dispute-job/'.$mile['id']); ?>" onsubmit="return edit_portfolio();">
				<div class="modal-body">
					<fieldset>
						<div id="milessss">
							<div class="row">
...
<div class="modal fade awaddd" id="request_cancel" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
										...
										<div class="row">
											<div class="col-sm-8">
												...
												<p><b>Milestone Amount</b><span style="float: right;"><i class="fa fa-gbp"></i><?php echo $mile['milestone_amount']; ?> GBP</span></p>
												 <p><b>Date Created</b><span style="float: right;"><?php $yrdata= strtotime($mile['cdate']);   echo date('d', $yrdata)." ".date('F', $yrdata); ?>, <?php echo date('Y',$yrdata); ?></span></p>
											</div>
										</div>
	...
	$check_disput_user = $this->common_model->GetColumnName('tbl_dispute',array('mile_id'=>$mile['id'],'disputed_by'=>$user_id2),array('ds_id'));
	if($check_disput_user){ ?>
	<a class="btn btn-primary btn-sm" onclick="return confirm('Are are sure, you want to cancel this dispute');" href="<?php echo site_url().'cancel_dispute/'.$check_disput_user['ds_id'].'/'.$_REQUEST['post_id']; ?>">Cancel Dispute</a> 
	<?php } ?>
			...
			<div class="col-sm-3">
				<div class="dashboard-white edit-pro89"> 
					<div class=" dashboard-profile Locations_list11">
						...
						<h4><?php $get_milestone_release=$this->common_model->get_milestone_paid($get_bid_post[0]['bid_by'],$_REQUEST['post_id']); ?><?php  if($get_milestone_release[0]['amount']){ echo "£".$get_milestone_release[0]['amount']; }else{ echo "£0.00"; } ?> GBP</h4>
						<br>
						<h5><b>Pending Milestones</b></h5>
						<h4><?php $get_milestone_pending=$this->common_model->get_milestone_pending($get_bid_post[0]['bid_by'],$_REQUEST['post_id']); ?><?php if($get_milestone_pending[0]['amount']){ echo "£".$get_milestone_pending[0]['amount']; }else{ echo "£0.00"; } ?> GBP</h4>
					</div>
        </div>
...
<script>
  ...
  function edit_portfolio(){
    $('.dispute_btn').html('<i class="fa fa-spin fa-spinner"></i> Processing...');
      ...
      payment: function(data, actions) {
        ...
        return actions.payment.create({
          payment: {
            transactions: [
              {
                amount: { total: amount_wp2, currency: 'GBP' }
              }
            ]
          }
        });
