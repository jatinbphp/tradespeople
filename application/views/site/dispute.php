				...
				<div class="col-sm-8">
					<div class="dispute">
						<div class="manage-profile">
							<h1 class="heading-inner d_lg_block">Milestone payment dispute</h1>
							<div class="inner-box">
...
<!--loop-->
				...
				<div class="col-sm-4 p-sm-0">
					<h1 class="heading-inner d_sm_block">Milestone payment dispute</h1>
						...
						<div class="bg-light p-3">
									...
									<div class="col-sm-6 text-center" style="border-left: 1px solid #c5c5c5;">
										...
										if ($dispute['ds_status'] == '0') {
											if ($dispute['tradesmen_offer'] && $dispute['offer_rejected_by_homeowner'] == 0) {
												...
												if($checkOtherUserReply){
													echo ' &nbsp;<a class="btn btn-danger btn-xs" onclick="return confirm(\'Are are sure, you want to reject this?\');" href="' . site_url() . 'reject_dispute_offer/?dispute_id=' . $dispute['ds_id'] . '&user_id=' . $user_id . '&job_id=' . $dispute['ds_job_id'] . '">Reject</a>';
												} else {
							...
							<?php if ($dispute['ds_status'] == '0') { ?>
								...
								<div class="row">
									...
									<form method="post" onsubmit="return checkValidation();" action="<?php echo site_url() . 'submit_dispute_offer/?dispute_id=' . $dispute['ds_id'] . '&user_id=' . $user_id . '&job_id=' . $dispute['ds_job_id'] ?>">
								...
								<?php if ($dispute['ds_status'] == '0') { ?>
									<div class="reply-btn text-center">
										<?php if ($dispute['disputed_by'] == $user_data['id']) { ?>
											<a class="btn btn-primary p-2 m-1" onclick="return confirm('Are are sure, you want to cancel this dispute?');" href="<?php echo site_url() . 'cancel_dispute/' . $dispute['ds_id'] . '/' . $dispute['ds_job_id']; ?>">Cancel Dispute</a>
										<?php } ?>
										<?php
										if ($checkOtherUserReply) {
											if ($showStepIn) {
	...
	<div class="modal fade" id="disputework" role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-content">
				...
				<form method="post" onsubmit="return check_from();" id="replyForm" action="<?php echo site_url('sen_comment'); ?>" enctype="multipart/form-data">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal">&times;</button>
