<?php include 'include/header.php'; ?>
<div class="home-page">
   <div class="home-demo first-home">
		<div class="owl-carousel banner-slider owl-loaded owl-drag">         
		  <div class="owl-stage-outer">
			  <div class="owl-stage">
				  <div class="owl-item active">
					  <div class="item">
							<div class="set-slide" style="background: url(<?=base_url()?>img/register-im.png);">
							   <svg width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 740 650" version="1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
								  <polygon opacity="0.9" fill="#FFFFFF" points="0 0 740 0 700 650 0 650"></polygon>
							   </svg>
							   <div class="container">
								  <div class="row set-m-10">
									 <div class="col-sm-10">
										<div class="row">
										   <div class="col-sm-7">
											  <div class="slice-text slice-text_home">
												 <p class="nomargin headHead1">Need more jobs to fill the gaps in your schedule?</p>
												 <p>Over 30,000 homeowners post jobs on our platform every single month, and we’re looking for tradespeople like you to do them.</p>
												 <div class="banner-btn margintop10">
													<a href="<?php echo base_url('signup-step1'); ?>" class="btn btn-warning btn-lg">Sign up for free</a>
												 </div>
											  </div>
										   </div>
										</div>
									 </div>
								  </div>
							   </div>
							</div>
						 </div>
					 </div>
				 </div>
			 </div>
		</div>
   </div>
</div>
<div class="register-pro">


<!-- Section-->
<div class="how-need">
	<div class="container">
  <div class="add-new-need">
  <h1 class="head-home"> Start winning jobs in a minute, not weeks.</h1>          
    <div class="row">
      <div class="col-sm-4 white-bg-need">
        <div class="need-new">
        <div class="new-top">     
		<div class="sign-svg">
		<svg height="577pt" viewBox="-18 -18 577.33246 577" width="577pt" xmlns="http://www.w3.org/2000/svg"><path style="fill:#fe8a0f" d="m473.847656 3.714844c-2.347656-2.347656-5.539062-3.652344-8.859375-3.6171878-3.308593.0234378-6.476562 1.3164058-8.855469 3.6171878l-178.664062 178.65625-.496094.503906-.25.25c-.246094.375-.621094.75-.871094 1.121094 0 .125-.125.125-.125.25-.25.371094-.378906.625-.625.996094-.128906.125-.128906.253906-.25.375-.125.375-.246093.625-.375 1 0 .125-.125.125-.125.246093l-31.816406 95.945313c-1.523437 4.464844-.363281 9.414062 3 12.726562 2.355469 2.324219 5.539063 3.625 8.851563 3.617188 1.355469-.027344 2.703125-.234375 3.996093-.625l95.695313-31.9375c.121094 0 .121094 0 .246094-.125.394531-.113282.777343-.28125 1.121093-.496094.097657-.015625.1875-.058594.253907-.128906.371093-.246094.871093-.496094 1.246093-.75.371094-.25.746094-.625 1.121094-.871094.128906-.121094.25-.121094.25-.25.125-.125.375-.25.503906-.496094l178.777344-178.785156c2.339844-2.335938 3.652344-5.5 3.652344-8.796875 0-3.300781-1.3125-6.46875-3.652344-8.796875zm-182.152344 210.597656 35.308594 35.308594-52.902344 17.589844zm58.390626 23.082031-46.164063-46.160156 161.066406-161.070313 46.160157 46.160157zm0 0"/><path d="m444.402344 233.277344c-6.882813.019531-12.457032 5.59375-12.476563 12.476562v233.183594c-.058593 20.644531-16.777343 37.363281-37.429687 37.429688h-332.113282c-20.644531-.066407-37.371093-16.785157-37.429687-37.429688v-332.121094c.058594-20.644531 16.785156-37.367187 37.429687-37.429687h233.175782c6.894531 0 12.476562-5.585938 12.476562-12.476563s-5.582031-12.476562-12.476562-12.476562h-233.175782c-34.449218.015625-62.367187 27.9375-62.382812 62.382812v332.121094c.015625 34.445312 27.933594 62.367188 62.382812 62.378906h332.113282c34.449218-.011718 62.371094-27.933594 62.382812-62.378906v-233.183594c-.019531-6.882812-5.59375-12.457031-12.476562-12.476562zm0 0"/></svg>
		</div>
          <h3 class="text-center">Create Profile</h3>
		  <p>Create a profile, choose the location you want to work in and get a list of homeowners who need your help.</p>
          </div>          
        </div>
      </div>
      <div class="col-sm-4 white-bg-need center-border">
        <div class="need-new">
        <div class="new-top"> 
		<div class="sign-svg">
		<svg id="Layer_1" enable-background="new 0 0 512 512" height="512" viewBox="0 0 512 512" width="512" xmlns="http://www.w3.org/2000/svg"><path style="fill:#fe8a0f" d="m287.744 368.986h-114.888c-5.561 0-10.069 4.509-10.069 10.071s4.508 10.071 10.069 10.071h114.888c5.561 0 10.069-4.509 10.069-10.071.001-5.562-4.508-10.071-10.069-10.071z"/><path style="fill:#fe8a0f" d="m119.127 430.688c0 5.562 4.508 10.071 10.069 10.071h56.212c5.561 0 10.069-4.509 10.069-10.071s-4.508-10.071-10.069-10.071h-56.212c-5.561 0-10.069 4.509-10.069 10.071z"/><path style="fill:#fe8a0f" d="m265.124 430.688c0 5.562 4.509 10.071 10.069 10.071h56.212c5.561 0 10.069-4.509 10.069-10.071s-4.509-10.071-10.069-10.071h-56.212c-5.56 0-10.069 4.509-10.069 10.071z"/><path style="fill:#fe8a0f" d="m230.201 420.617c-5.561 0-10.069 4.509-10.069 10.071s4.508 10.071 10.069 10.071h.198c5.561 0 10.069-4.509 10.069-10.071s-4.508-10.071-10.069-10.071z"/><path style="fill:#fe8a0f" d="m25.811 166.535c5.561 0 10.069-4.509 10.069-10.071v-.199c0-5.562-4.508-10.071-10.069-10.071s-10.069 4.509-10.069 10.071v.199c0 5.562 4.508 10.071 10.069 10.071z"/><path d="m484.409 397.297-45.244-45.243v-255.015c0-.322-.018-.642-.049-.961-.013-.134-.036-.265-.053-.397-.024-.179-.048-.357-.081-.534-.031-.16-.07-.317-.108-.474-.035-.144-.07-.288-.111-.43-.048-.166-.102-.329-.159-.492-.045-.13-.092-.26-.142-.389-.063-.161-.13-.32-.202-.477-.058-.128-.12-.255-.183-.381-.075-.148-.152-.296-.234-.44-.075-.132-.155-.26-.236-.389-.083-.131-.164-.262-.253-.389-.096-.137-.198-.27-.301-.403-.086-.111-.17-.223-.26-.331-.025-.03-.046-.061-.071-.091l-74.968-86.966c-.064-.074-.137-.139-.203-.211-.144-.157-.289-.313-.443-.461-.103-.099-.209-.193-.316-.287-.153-.135-.308-.267-.469-.393-.11-.086-.222-.169-.335-.251-.172-.123-.348-.24-.527-.353-.109-.068-.218-.136-.329-.2-.198-.113-.401-.218-.607-.318-.102-.049-.201-.101-.304-.147-.227-.101-.459-.19-.694-.274-.091-.033-.179-.069-.27-.099-.252-.082-.51-.15-.77-.213-.081-.02-.161-.045-.242-.062-.273-.059-.552-.101-.833-.136-.072-.009-.143-.025-.215-.033-.352-.036-.708-.057-1.069-.057h-293.355c-24.83 0-45.031 20.217-45.031 45.068v66.31c0 5.562 4.508 10.071 10.069 10.071s10.069-4.509 10.069-10.071v-66.31c0-13.745 11.167-24.926 24.893-24.926h283.286v64.911c0 12.163 9.906 22.058 22.082 22.058h52.885v224.805l-22.119-22.119c-.055-.058-.103-.12-.159-.176-.057-.057-.119-.105-.177-.16l-47.479-47.478c9.714-18.551 15.219-39.639 15.219-61.992 0-73.899-60.11-134.02-133.996-134.02-73.901 0-134.023 60.121-134.023 134.02 0 35.273 13.699 67.401 36.046 91.36.46.615.992 1.178 1.591 1.68 24.389 25.267 58.582 41.009 96.387 41.009 22.34 0 43.415-5.505 61.955-15.218l47.462 47.48c.057.06.106.124.165.182.058.058.122.107.181.164l78.948 78.979v21.306c0 13.745-11.167 24.926-24.893 24.926h-333.361c-13.726 0-24.893-11.182-24.893-24.926v-265.78c0-5.562-4.508-10.071-10.069-10.071s-10.069 4.509-10.069 10.071v265.779c0 24.851 20.201 45.068 45.031 45.068h333.361c24.83 0 45.031-20.217 45.031-45.068v-4.174c5.518 2.828 11.116 4.085 16.525 4.085 10.709-.001 20.677-4.881 27.804-12.014 14.161-14.17 19.294-39.149.915-57.532zm-120.211-312.244v-47.878l42.922 49.794h-40.978c-1.054 0-1.944-.878-1.944-1.916zm-133.883 1.059c62.781 0 113.857 51.086 113.857 113.878 0 20.808-5.611 40.329-15.391 57.134-.711.842-1.265 1.771-1.656 2.751-1.982 3.194-4.117 6.284-6.395 9.258-11.971-22.144-31.484-39.042-54.579-47.918 13.643-10.97 22.399-27.785 22.399-46.616 0-32.991-26.823-59.832-59.793-59.832-32.985 0-59.821 26.84-59.821 59.832 0 18.811 8.741 35.612 22.367 46.583-7.933 3.046-15.515 7.061-22.591 12.032-12.661 8.896-23.049 20.312-30.628 33.515-13.614-18.769-21.655-41.831-21.655-66.74 0-62.791 51.089-113.877 113.886-113.877zm-41.242 88.488c0-21.885 17.802-39.69 39.683-39.69 21.866 0 39.655 17.805 39.655 39.69 0 21.87-17.788 39.662-39.655 39.662-21.881 0-39.683-17.793-39.683-39.662zm-36.74 108.323c13.789-29.268 43.725-48.52 76.423-48.52 33.651 0 63.944 20.065 77.307 50.549-4.93 4.404-10.245 8.386-15.886 11.889-.992.394-1.932.955-2.783 1.676-16.792 9.774-36.293 15.38-57.08 15.38-30.145 0-57.588-11.78-77.981-30.974zm171.756 12.719c.692-.533 1.304-1.148 1.827-1.827 4.392-4.478 8.474-9.26 12.209-14.315l37.253 37.252-28.353 28.359-37.244-37.259c5.053-3.736 9.833-7.818 14.308-12.21zm145.162 144.947c-6.468 6.473-17.078 9.695-27.454-.671l-80.534-80.565 28.356-28.361 32.17 32.17c.127.133.256.263.39.39l47.99 47.989c10.315 10.317 5.103 23.024-.918 29.048z"/></svg>
		</div>
          <h3 class="text-center">Pick your job</h3>
		  <p>Access list of jobs and provide quotes for a small fee taken off your credit. Discuss details.Milestone payment created.</p>
        </div>
        </div>
      </div>
      <div class="col-sm-4 white-bg-need">
        <div class="need-new">
        <div class="new-top">
		<div class="sign-svg">
		<svg height="496pt" viewBox="0 0 496 496" width="496pt" xmlns="http://www.w3.org/2000/svg"><path d="m456.007812 159.992188.007813-40.015626-39.960937.0625.035156-39.976562-40.019532.007812v-40.015624h-40.023437l-.039063-40.054688h-51.328124l-284.679688 284.6875v51.34375l39.992188-.023438-.007813 40.015626 39.960937-.0625-.035156 39.976562 40.019532-.007812v40.007812l40.070312.054688-.023438 40.007812h51.335938l284.6875-284.6875v-51.34375zm23.992188 44.695312-275.3125 275.3125h-28.703125l.015625-39.992188-40.078125-.054687v-40.023437l-40 .007812.03125-40.007812-39.96875.0625.007813-40-40 .023437v-28.703125l275.320312-275.3125h28.710938l.03125 40.054688h40.007812v40.015624l40-.007812-.03125 40.007812 39.96875-.0625-.007812 40 40-.023437v28.703125zm0 0"/><path d="m221.65625 221.65625-11.3125-11.3125-10.34375 10.34375-16-16-24 24c-5.160156 5.152344-8 12.007812-8 19.3125s2.839844 14.167969 8 19.3125l4.679688 4.6875c2.144531 2.144531 3.320312 4.976562 3.320312 8 0 2.984375-1.207031 5.894531-3.3125 8l-12.6875 12.6875-12.6875-12.6875c-2.105469-2.105469-3.3125-5.015625-3.3125-8s1.207031-5.894531 3.3125-8l10.34375-10.34375-11.3125-11.3125-10.34375 10.34375c-5.160156 5.152344-8 12.007812-8 19.3125 0 5.710938 1.785156 11.121094 5.007812 15.679688l-10.664062 10.664062 11.3125 11.3125 10.34375-10.34375 16 16 24-24c5.160156-5.152344 8-12.007812 8-19.3125 0-7.289062-2.839844-14.152344-8-19.3125l-4.6875-4.6875c-2.105469-2.105469-3.3125-5.015625-3.3125-8s1.207031-5.894531 3.3125-8l12.6875-12.6875 12.679688 12.6875c2.144531 2.144531 3.320312 4.976562 3.320312 8 0 2.984375-1.207031 5.894531-3.3125 8l-10.34375 10.34375 11.3125 11.3125 10.34375-10.34375c5.160156-5.152344 8-12.007812 8-19.3125 0-5.703125-1.785156-11.105469-5.007812-15.671875zm0 0" style="fill:#fe8a0f"/><path d="m192 140.6875-147.3125 147.3125 99.3125 99.3125 147.3125-147.3125zm-124.6875 147.3125 124.6875-124.6875 76.6875 76.6875-124.6875 124.6875zm0 0"/><path d="m253.105469 79.585938 163.308593 163.308593 34.898438-34.894531-163.3125-163.3125zm163.308593 140.6875-140.6875-140.6875 12.273438-12.273438 140.6875 140.6875zm0 0"/><path d="m234.339844 101.660156 11.316406-11.316406 159.996094 160-11.3125 11.3125zm0 0"/><path d="m210.339844 125.660156 11.316406-11.316406 159.996094 160-11.3125 11.3125zm0 0"/><path d="m290.335938 261.65625 11.316406-11.316406 47.996094 48-11.3125 11.3125zm0 0"/><path d="m266.335938 285.65625 11.316406-11.316406 47.996094 48-11.3125 11.3125zm0 0"/><path d="m242.335938 309.65625 11.316406-11.316406 47.996094 48-11.3125 11.3125zm0 0"/><path d="m218.335938 333.652344 11.316406-11.3125 47.996094 48-11.3125 11.3125zm0 0"/><path d="m194.335938 357.652344 11.316406-11.3125 47.996094 48-11.3125 11.3125zm0 0"/><path d="m170.335938 381.652344 11.316406-11.3125 47.996094 47.996094-11.3125 11.316406zm0 0"/></svg>
		</div>
          <h3 class="text-center">Get paid</h3>
		  <p>Job done as agreed. Milestone released and payment made. Share your experience and let other tradespeople know what it is like working with your homeowner.</p>
        </div>
        </div>
      </div>
    </div>
  </div>
</div>
</div>

<!-- Section-->
<div class="full-reimg">
	<div class="container">
		<div class="row">
			<div class="col-sm-9">				
				<h1 class="pull-left">Grow your business and say goodbye to gaps in your schedule</h1> 
				
			</div>
			<div class="col-sm-3">
				<a style="" href="<?php echo base_url('signup-step1'); ?>" class="btn btn-warning btn-lg">Sign up now</a>
			</div>
		</div>
	</div>
</div>
<div class="newreg-section">
   <!-- section 1 -->
   <div class="how-need reg-1 left-img">
	<div class="container">
		<div class="row">
		<div class="col-sm-10 col-sm-offset-1">	
		<div class="row">
			<div class="col-sm-4">
				<div class="set-need set-post">
					<img src="<?=base_url()?>img/countless_job.png">
				</div>
			</div>
			<div class="col-sm-8">	
				<div class="pad_us1">
					<div class="need-text">
					<div class="need-box">
					<h1 class="head-home">Countless local job opportunities</h1>
					<p>Tradespeoplehub provides easy access and connection to over 1000 local jobs. If you’re in search of trade work near you, look no further as we’ve got enough for you. Simply sign-up for free to start doing them.</p>
					</div>
				</div>
				</div>
			</div>
		</div>
		</div>
		</div>
	</div>
	</div>
   <!-- section 1 -->
   <!-- section 2 -->
   <div class="how-need reg-2 right-img">
	<div class="container">
		<div class="row">
		<div class="col-sm-10 col-sm-offset-1">	
		<div class="row">
			<div class="col-sm-8">	
				<div class="pad_us1">
					<div class="need-text">
					<div class="need-box">
					<h1 class="head-home">We’ve got your back</h1>
					<p>Unlike directories, we help you access the best job, guaranteed to maximize your returns on investment and experience real business growth. That way, you can concentrate on delivering your best service instead of endless searches for potential clients.</p>
					</div>
				</div>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="set-need set-post">
					<img src="<?=base_url()?>img/your_back.png">
				</div>
			</div>
		</div>
		</div>
		</div>
	</div>
	</div>
   <!-- section 2 -->
   <!-- section 3 -->
   <?php if($settings[0]['payment_method'] == 1){?>
		  <div class="how-need reg-3 left-img">
				<div class="container">
					<div class="row">
					<div class="col-sm-10 col-sm-offset-1">	
					<div class="row">
						<div class="col-sm-4">
							<div class="set-need set-post">
								<img src="<?=base_url()?>img/Pay_as.png">
							</div>
						</div>
						<div class="col-sm-8">	
							<div class="pad_us1">
								<div class="need-text">
								<div class="need-box">
								<h1 class="head-home">Pay as you go option</h1>
								<p>Unlike other sites that charge a compulsory monthly fee regardless of time, we offer a pay-as-you-go option that allows you to pick what works best for you; this pricing model means you only pay when you want. You have complete flexibility – there are no contracts or long-term commitments.</p>
								</div>
							</div>
							</div>
						</div>
					</div>
					</div>
					</div>
				</div>
			</div>
   <!-- section 3 -->
   <!-- section 4 -->
	  <div class="how-30_day reg-4 right-img">
			<div class="container">
				<div class="row">
				<div class="col-sm-10 col-sm-offset-1">	
				<div class="row">
					<div class="col-sm-8">	
						<div class="pad_us1">
							<div class="need-text">
							<div class="need-box">
							<h1 class="head-home">30-day free trial </h1>
							<p>We are confident about our service; as a result, we’re offering a free trial to enable you to try our service for free, and with no financial commitments. With this, you can be accustomed to our platform, decide if it’s right for you and pick what works best for you.</p>
							</div>
						</div>
						</div>
					</div>
					<div class="col-sm-4">
						<div class="set-need set-post">
							<img src="<?=base_url()?>img/30DAY.png">
						</div>
					</div>
				</div>
				</div>
				</div>
			</div>
		</div>
   <!-- section 4 -->

		<div class="white-reg  Start_30_Da" >
			<div class="container">
				<div class="row">
					<div class="col-sm-9">
						<h1>Start a free 30-day trial today and see how you can increase your client database. </h1>
					</div>
					<div class="col-sm-3">
						<a href="https://www.tradespeoplehub.co.uk/signup-step1" class="btn btn-warning btn-lg">Start your free trial now</a>
					</div>
				</div>
			</div>
		</div>
	<?php }?>	
   <!-- section 5 -->
   <div class="how-need reg-4 left-img" >
	<div class="container">
		<div class="row">
		<div class="col-sm-10 col-sm-offset-1">	
		<div class="row">
			<div class="col-sm-4">
				<div class="set-need set-post">
					<img src="<?=base_url()?>img/payment-done.png">
				</div>
			</div>
			<div class="col-sm-8">	
				<div class="pad_us1">
					<div class="need-text">
					<div class="need-box">
					<h1 class="head-home">Payment guarantee</h1>
					<p>Our project milestones protect your interest and that of your committed homeowner. So, you don’t have to worry about being owed after completing your job. It also shows the commitment of your client when they create them.</p>
					</div>
				</div>
				</div>
			</div>
			
		</div>
		</div>
		</div>
	</div>
	</div>
   <!-- section 5 -->
</div>

<div class="white-reg">
	<div class="container">
		<div class="row">
			<div class="col-sm-9">
				<h1>Join over 20,000 tradespeople across the UK to experience true business performance increase. </h1>
			</div>
			<div class="col-sm-3">
				<a href="<?php echo base_url('signup-step1'); ?>" class="btn btn-warning btn-lg">Join for free now</a>
			</div>
		</div>
	</div>
</div>


<!-- Testimonial-->
<div class="highlight testimonials">
    <div class="container">

        <div class="section-title">
            <h2 class="head-home">Don´t take our word for it</h2>
        </div>
		<div class="row">
		<div class="col-sm-10 col-sm-offset-1">
        <section class="row breath">
            <div class="col-md-6">
                <div class="testblock">Whenever I have last minute cancellations or some time to spare, Tradespeople hub has always found me jobs that fill my schedule</div>
                <div class="clientblock"> <img src="<?=base_url()?>img/testimonial_1.png" alt=".">
                    <p><strong>Johnathan Doe</strong>
                        <!--<br> Founder and CEO of <a href="#">Company Inc</a>--></p>
                </div>
            </div>
            <div class="col-md-6">
                <div class="testblock">Tradespeople hub is a life-saver.They've saved me from  the high cost associated with advertising and promoting my business online</div>
                <div class="clientblock"> <img src="<?=base_url()?>img/testimonial_2.png" alt=".">
                    <p><strong>Michael Slats</strong>
                        <!--<br> Project Lead at <a href="#">Envato.com</a>--></p>
                </div>
            </div>
        </section>
		</div>
		</div>
    </div>
</div>
<!-- Testimonial-->



<!-- our Team-->
<div class="our-team">
	<div class="container">
        <h2 class="head-home">Our team</h2>
		<div class="row">
			<div class="col-sm-3">
				<div class="team-member">
					<img src="<?=base_url()?>img/team1.png">
				</div>
			</div>
			<div class="col-sm-3">
				<div class="team-member">
					<img src="<?=base_url()?>img/team2.png">
				</div>
			</div>
			<div class="col-sm-3">
				<div class="team-member">
					<img src="<?=base_url()?>img/team3.png">
				</div>
			</div>
			<div class="col-sm-3">
				<div class="team-member">
					<img src="<?=base_url()?>img/team4.png">
				</div>
			</div>
		</div>
	</div>
</div>
<!-- our Team-->

<!-- most-say-->
<div class="most-say">
	<div class="container">
		<h1 class="head-home">The most simple and yet powerful  way to grow your business</h1>
		<a href="<?php echo base_url('signup-step1'); ?>" class="btn btn-warning btn-lg">Sign up & trial us for free now</a>
	</div>
</div>
<!-- most-say-->
</div>


<?php include 'include/footer.php'; ?>