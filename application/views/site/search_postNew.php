			...
			?>
							...
							<div class="pull-right">
								...
								<div class="from-group text-right" style="margin-top: 10px;">
									<?php if($this->session->userdata('user_id')){ if($this->session->userdata('type')==1){ ?><a href="<?php echo base_url('details/?post_id='.$jobs['job_id']); ?>" class="btn btn-warning">Quote Now</a>
								<?php } }else{ ?>
				...
				<div class="Locations_list11 fat-side-item open_omon2">
					...
					<ul class="ul_set lislocc1">
						...
						foreach ($get_all_categories as $c) {
							...
							<li><a href="<?php echo base_url('find-jobs/'.$c['slug']); ?>"><?php echo $c['cat_name']; ?></a>
								<ul>
								<?php foreach ($child as $cc) { ?>
									<li><a href="<?php echo base_url('find-jobs/'.$cc['slug']); ?>"><?php echo $cc['cat_name']; ?></a></li>
								<?php } ?>
								</ul>
							</li>
						<?php } ?>
