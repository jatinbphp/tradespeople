				...
				<div class="dashboard-white">
					...
					<div class="dashboard-profile dhash-news">
						...
						<div class="pull-right">
							<h3 class="top_us_d1"><?php echo ($project_details['budget']) ? '£' . $project_details['budget'] : ''; ?><?php echo ($project_details['budget2']) ? ' - £' . $project_details['budget2'] : ''; ?></h3>
							<div class="from-group text-right">
								...
								<?php if ($this->session->userdata('user_id')) {
                                    ...
                                    if ($get_post_bid[0]['bid_by'] == $this->session->userdata('user_id')) {?>

										<script type="text/javascript">
										setInterval(function(){
											get_unread_msg_count(<?php echo $_REQUEST['post_id']; ?>,<?php echo $get_post_bid[0]['posted_by']; ?>);
										},5000);
										</script>
									<?php }}?>
							</div>
					...
					<div class="dhash-news-main job-description-detail">
						<div class="job-description-detail">
							<p>
								<?php echo $project_details['description']; ?>
							</p>
						</div>
						<?php if ($project_details['category'] != '') {
                            ?>
						<h5><b>Skill Required</b></h5>
						<div class="services">
							<?php
                                $selected_lang = explode(',', $project_details['subcategory_1']);
...
<?php }?>
						...
						<div class="Required_mne">
							<button class="btn btn-primary pull-left" style="display: none;">RECRUITER</button>
							<a href="#" class="pull-left" style="font-size: 12px;">Posted:							                                                              							                                                              							                                                              							                                                              							                                                              							                                                              							                                                               <?php echo date('d-m-Y', strtotime($project_details['c_date'])); ?></a>
							<a href="#" class="pull-right" style="font-size: 12px;">Project ID:							                                                                   							                                                                   							                                                                   							                                                                   							                                                                   							                                                                   							                                                                    <?php echo $project_details['project_id']; ?></a>
						</div>
						<div class="Required_mne" style="margin-top: 0;">
							<a href="javascript:void(0)" onclick="reportJobPopup(<?php echo $project_details['job_id'] ?>)" style="font-size: inherit;display: inline-block;color: #3d78cb;float: right;">Report this job</a>
						</div>
						...
						<div class="Attachment2">
							<a href="<?php echo base_url('img/jobs/' . $a['post_doc']); ?>" download><i class="fa fa-paperclip"></i><?php echo $a['post_doc']; ?></a>
						</div>
						<?php }}}?>
        ...
        <div class="msg1"><?=$this->session->flashdata('msg1');?></div>
				...
				<?php
                    ...
                    if ($this->session->userdata('type') == 1) {
                        ...
                        $datesss = date('Y-m-d H:i:s', strtotime($project_details['c_date'] . ' + ' . $closed_date . ' days'));
                        if (date('Y-m-d H:i:s') >= $datesss && $get_post_bid == 0) {
                        ?>
				...
				<?php } else if ((count($get_post_bid) > 0 && $get_post_bid[0]['status'] == 0) && ($project_details['status'] != 4 && $project_details['status'] != 7 && $project_details['status'] != 5) && (date('Y-m-d H:i:s') < $datesss)) {
                            ...
                            $get_bid_user = $this->common_model->get_single_data('users', ['id' => $get_post_bid[0]['bid_by']]);

                            $get_user_suggest = $this->common_model->get_all_data('tbl_milestones', ['bid_id' => $get_post_bid[0]['id']]);
                        ?>
									...
									<div class="col-sm-6">
										<div class="from-group">
											...
											<div class="input-group">
												...
												<input class="form-control" placeholder="4" type="number" name="bid_amount" value="<?php echo $get_post_bid[0]['bid_amount']; ?>" required=""><span class="input-group-addon" >GBP</span>
											</div>
									 </div>
									</div>
									<div class="col-sm-6">
...
<?php }?>
                ...
                <!--
								<div class="suggest_mile" id="field1">
                  <div class="row">
										<div class="col-sm-6">
											<div class="from-group">
											 <input type="text" class="form-control miname_1" name="tsm_name[]" placeholder="Define the tasks that you will complete for this" required>
											</div>
										</div>
										<div class="col-sm-4">
											<div class="from-group input-group">
												<span class="input-group-addon"><i class="fa fa-gbp"></i></span>
												<input type="number" class="form-control miamount_1" min="1" name="tsm_amount[]" required>
											</div>
										</div>
										<div class="col-sm-2">
											<div class="form-group">
						...
						<h2>Recent reviews</h2>
							<div class="min_h3">
								<?php foreach ($get_reviews as $serialNumber => $r) {
                                            $job_title   = $this->common_model->GetColumnName('tbl_jobs', ['job_id' => $r['rt_jobid']], ['title', 'direct_hired']);
                                            $get_users22 = $this->common_model->GetColumnName('users', ['id' => $r['rt_rateBy']], ['trading_name']);
                                        ?>
    ...
    success:function(resp){
      if(resp.status==1){
        ...
        window.location.href='<?php echo base_url('proposals?post_id=' . $_GET['post_id']); ?>';
      	...
      	swal({
		    title: "",   
		    text: "Contact detail detected. Remove it and use the chat feature to contact the client.",
		    type: "error" 
		});
...
<?php
    if ($this->session->userdata('user_id') && $this->session->userdata('type') == 1) {
        ...
        $check_visit = $this->common_model->GetColumnName('visit_job', ['job_id' => $_REQUEST['post_id'], 'user_id' => $user_id], ['id']);
        if (!$check_visit) {
            $this->common_model->insert('visit_job', ['job_id' => $_REQUEST['post_id'], 'user_id' => $user_id, 'visit_date' => date('Y-m-d')]);
        }
}?>
