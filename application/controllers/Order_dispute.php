	...
	public function cancel_dispute($id = null, $post_id = null)
	{
		...
		if ($id && $post_id && $data) {
			...
			if ($run) {
				...
				/* CHECK ORDER IS CUSTOM ORDER */
				if($job['is_custom'] == 1 && $job['order_type'] == "milestone"){

					$this->common_model->update_data('tbl_milestones', array('dispute_id' => $id), array('status' => 10, 'service_status' => 'delivered', 'service_previous_status' => 'disputed',  'dispute_id' => NULL));
