...
class Posts extends CI_Controller
{
	...
	public function cancel_requested_milestone($id,$postid){
		...
		if($has_email_noti){
			...
			$this->common_model->send_mail($has_email_noti['email'],$subject,$content,null,null,'support');
	...
	public function decline_requested_milestone($id,$postid){
		...
		if($has_email_noti){
			...
			$sent = $this->common_model->send_mail($has_email_noti['email'],$subject,$html,null,null,'support');
		}
	...
	public function job_a_post() {
		...
		if($this->form_validation->run()==false){
			...
			$insert['postal_code'] = str_replace(" ","",$this->input->post('post_code'));
			...
			if($run){
				...
				$insert1['post_code']=str_replace(" ","",$this->input->post('post_code'));
		...
		if($json['status']==1){
			...
			$this->common_model->send_mail($this->session->userdata('email'), $subjectToHomeOwner, $contentToHomeOwner,null,null,'support');
			/* Email confirmation to homeowner */


			
			$trades = $this->common_model->GetColumnName('users',"type = 1 and u_email_verify = 1 and notification_status = 1 and FIND_IN_SET($category,category)",array('phone_no','id','f_name','email'),true);
			...
			if(count($trades)>0){
				...
				foreach($trades as $key => $value){
					...
					$subject = "New ".$cateName['cat_name']." Job Posted: ".$userdataaa['city'].": Quote now!";
					...
					$html .= '<table border="0" cellpadding="0" cellspacing="0" width="100%"><tr><td style="border:1px solid #2875D7; background:#2875d7; color:#fff;  padding:10px 15px; width: 75%">Job title</td><td style="border:1px solid #2875D7; background:#2875d7; color:#fff;  padding:10px 15px; width: 25%;">Budget</td></tr>';
	...
	public function accept_post($uid,$status,$amount,$check_create_ms){
		...
		if($check) {
			...
			$this->common_model->send_mail($user['email'],$subjectH,$htmlH,null,null,'support');
  ...
  echo json_encode($json);
	...
	public function accept_award123($uid,$status) {
		...
		if($check){
			...
			$this->common_model->send_mail($get_posted_user['email'],$subjectH,$htmlH,null,null,'support');
			...
			if($get_post_data && $get_job_post){
				...
				if($tsm_amount1 > $get_users['u_wallet']) {
					...
					if($has_email_noti1){
						...
						$sent = $this->common_model->send_mail($has_email_noti1['email'],$subject,$html,null,null,'support');
					}
	...
	$get_user_details=$this->common_model->get_single_data('users',array('id'=>$get_milestones[0]['userid']));
			...
			$this->common_model->send_mail($get_users['email'],$subject,$html,null,null,'support');
...
public function edit_post($id) {
		...
		if ($this->form_validation->run()==false) {
			...
			if($check_postcode['status']==1){
				...
				$insert['budget'] = $this->input->post('price');
...
public function reject_award($post_id,$job_id) {
		...
		$this->common_model->send_mail($homeOwner['email'],$subject,$html,null,null,'support');
...
public function reject_post($post_id,$job_id) {
		...
		if($result) {
			...
			$runs1=$this->common_model->send_mail($get_posted_user['email'],$subject,$content,null,null,'support');
	...
	$u_name=$get_userss['f_name'].' '.$get_userss['l_name'];
		...
		$subject = "Request for cancellation of milestone has been approved"; 
				...
				$content.='<p class="text-center">This is the mail to inform you that your request for cancelling the milestone name '.$get_milestones['milestone_name'].' of £'.$get_milestones['milestone_amount'].' has been approved by <a href="'.site_url().'profile/'.$get_users['id'].'">'.$get_users['trading_name'].'</a>.</p><p></p>';
				 $run = $this->common_model->send_mail($get_userss['email'],$subject,$content,null,null,'support');
		if($run)
		{
			$json['status'] = 1;
