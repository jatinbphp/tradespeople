...
public function getServiceDetails(){
  ...
  $lId = $data['service_details']['location'];
  $data['location'] = $this->Common_model->GetSingleData('location',['id'=>$lId]);
...
public function completed_orders(){
  ...
  $result['counter'] = $this->orderCounter('completed');
...
public function pending_orders(){
  ...
  $result['counter'] = $this->orderCounter('placed');
  $result['menu'] = 'Pending Orders';
  $result['service_fee'] = 'hide';
...
public function cancel_orders(){
  ...
  $result['counter'] = $this->orderCounter('cancelled');
...
public function service_category(){
  ...
  $result['price_per_type'] = ["Hour","Service","Meter","Square Meter","Kilogram","Mile","Consultation"];
...
public function getServiceCategoryLists(){
  ...
  foreach ($memData as $member) {
    ...
    $data[] = [$member->cat_id, $main_cate, $member->slug, $action];
  }
  ...
  /*if ($categories) {
      ...
      if ($fileError) {
        ...
        if ($result) {
          ...
          if(!empty($attribute)){
            ...
            $old_attribute = $this->Common_model->get_all_data('service_attribute', ['service_cat_id' => $id]);
            $old_attributes_map = [];
          ...
          $exService = $this->input->post('exService');
          if(!empty($exService['name'])){
            $old_exs = $this->Common_model->get_all_data('extra_service', ['category' => $id]);
            $old_exs_map = [];
            if(!empty($old_exs)){
    ...
    echo json_encode($json);
      ...
      public function openModal(){
        ...
        $selected_parent_category=$this->Common_model->get_parent_category('service_category');

        $spcId = [];
        if(!empty($selected_parent_category)){
          foreach($selected_parent_category as $cId){
            $spcId[] = $cId['original_cat_id'];
          }
        }

        $data['selected_parent_category'] = $spcId;
        $data['formType'] = 0;
        $data['catId'] = 0;

        if($catId > 0){
          $data['catId'] = $catId;
          $sCategory = $this->Common_model->GetSingleData('service_category', ['cat_id' => $catId]);
          $data['subCategory'] = $this->Common_model->GetSingleData('service_category', ['cat_parent' => $catId]);
          $service_type = $this->Common_model->get_all_data('service_category', ['cat_parent' => $data['subCategory']['cat_id']]);

          $service_type_name = '';

          if(!empty($service_type)){
            foreach($service_type as $type){
              $service_type_name .= $type['cat_name'].',';
            }
          }
          ...
          $exServices = $this->Common_model->get_all_data('extra_service',['category'=>$catId]);
          $exServicesList = '';
          if(!empty($exServices)){
            $n = 1;
          ...
          $data['exServicesList'] = $exServicesList;
          $data['formType'] = 1;
        }
        $view_content = $this->load->view('Admin/service_category_modal', ['data' => $data], true);

        $this->output->set_content_type('application/json')->set_output(json_encode(['html' => $view_content]));
      }

      public function create_service_category_slug($id=null, $title=null, $type=0){
        $value = empty($title) ? $this->input->post('title') : $title;
        $slug = url_title($value);      
        $slugcheck = $this->Common_model->GetColumnName('service_category',array('cat_id != '=>$id,'cat_name'=>$value),array('slug') , false , 'cat_id' , 'desc');
        if($slugcheck){
      ...
      public function updateStatus(){
        ...
        if(!empty($service)){
          ...
          if($status=='active'){
            ...
            $ntMessage = 'Your listing is now live. <a href="'.$pageUrl.'">View now!</a>';

            $subject1 = "Your Listed Service Gig is Now Live!"; 
                
            $content1 = '<p style="margin:0;padding:10px 0px">Dear '.$tradesMan['trading_name'].', </p>';
            
            $content1 .= '<p style="margin:0;padding:10px 0px">Your listing has been successfully approved by our team and is now live on the platform. Your service or gig is officially visible to potential customers to order, and we are excited to see how it performs.</p>';
            
            $content1.= '<div style="text-align:center"><a href="'.$pageUrl.'" style="background-color:#fe8a0f;color:#fff;padding:8px 22px;text-align:center;display:inline-block;line-height:25px;border-radius:3px;font-size:17px;text-decoration:none">View Listing</a></div>';

            $content1.='<p style="margin:0;padding:10px 0px">If you need any further assistance or have any questions regarding the gig, feel free to reach out. We\'re here to help!</p>';
          ...
          if(in_array($status,['denied','required_modification'])){
            $ntMessage = 'Your listing is now live. <a href="'.$pageUrl.'">View now!</a>';

            $subject1 = "Action Required: Listing Rejected and Modifications Required!"; 
                
            $content1 = '<p style="margin:0;padding:10px 0px">Dear '.$tradesMan['trading_name'].', </p>';
            
            $content1 .= '<p style="margin:0;padding:10px 0px">The listing you recently submitted on our platform has been reviewed and unfortunately rejected by the team. In order to proceed with the listing your service, some modifications and additional information are required.Please review the following points that need to be addressed:</p>';
            
            $content1.= '<div style="text-align:center"><a href="'.$pageUrl.'" style="background-color:#fe8a0f;color:#fff;padding:8px 22px;text-align:center;display:inline-block;line-height:25px;border-radius:3px;font-size:17px;text-decoration:none">Review Now</a></div>';

            $content1.='<p style="margin:0;padding:10px 0px">Once these changes have been made, kindly resubmit your gig for further review. We appreciate your understanding and cooperation in ensuring that your listing meets the platform’s guidelines.</p>';

            $content1.='<p style="margin:0;padding:10px 0px">If you need any assistance or have questions about the modifications, please do not hesitate to reach out.</p>';

            $content1.='<p style="margin:0;padding:10px 0px">Thank you for your prompt attention to this matter. We look forward to seeing your updated gig listed on the platform soon.</p>';
