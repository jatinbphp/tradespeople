...
class Home extends CI_Controller
{
  ...
  public function index(){
	...
	public function find_tradesmen(){
		$this->load->view('site/find-tradesmen');
	}
  ...
  public function signup_step1(){
    if($this->session->userdata('user_id')){
      ...
      $this->load->view('site/signup-step1');
    }
  }
  ...
  public function get_subcategories(){
    ...
    $data_set=$this->Common_model->newgetRows('category',array('cat_parent'=>$_REQUEST['val'],'is_delete'=>0));
		...
		$json['label']='What type of '.$cate_data['cat_name'].' service do you need?';
	...
	public function tradesman_help(){
		$this->load->view('site/tradesman-help');
	}
  ...
  public function submit_signup4() {
    ...
    if($this->form_validation->run()==false){
      ...
      /* Send email */
      $this->send_verification_email($insert['email'], "Verify your Email Address - Tradespeoplehub.co.uk", false, $insert['u_token']);
      //$this->send_verification_welcome_email($insert['email'], $insert['f_name'], $insert['u_token']);
  ...
  public function submit_signup7() {
    ...
    if($verification_code == $user_data['phone_code']){
      ...
      $run = $this->Common_model->update('users', array('id' => $user_id), $update);
			...
			$sent = $this->Common_model->send_mail($user_data['email'],$subject,$html,null,null,'support');
				...
				if($data['u_email_verify'] == 0){
					if($data['u_token'] == $token){
						...
						if($run) {
							...
							$sent = $this->Common_model->send_mail($data['email'],$subject,$html,null,null,'support');
      ...
      if($userdata['u_email_verify'] == 1){
        ...
        if($run){
          ...
          $html = '<p style="margin:0;font-size:20px;padding-bottom:5px;color:#2875d7">Please verify your email address.</p>
						<p style="margin:0;padding:10px 0px">Thanks for registering with Tradespeople Hub! To finish your sign up, please verify your email address by clicking the button below.</p>
						<div style="text-align:center"><a href="'.site_url().'email-verified/'.$id.'/'.$update['u_token'].'" style="background-color:#fe8a0f;color:#fff;padding:8px 22px;text-align:center;display:inline-block;line-height:25px;border-radius:3px;font-size:17px;text-decoration:none">Verify Email Address</a></div>';
					
          $sent = $this->Common_model->send_mail($userdata['email'],$subject,$html,null,null,'support');
  ...
  public function memberships(){
	...
	public function membership_plans(){
		if(!$this->session->userdata('user_id')){
			...
			if($data['user_profile']['free_trial_taken']==1){
		...
		if ($this->form_validation->run()==false) {
			...
			if($data) {
				if($data['status']==0){
					...
					$run = $this->Common_model->send_mail($data['email'],$subject,$contant,null,null,'support');
    ...
    if($responseData->success)
     { 
		...
		if($result1){
			...
			$result=$this->Common_model->send_mail($to,$sub,$body,null,null,'support');
			$json['status'] = 1;
  ...
  public function post_job(){
    ...
    $where['type'] = 2;
    $data['users'] = $this->Common_model->fetch_records('users', $where, false, false, 'document_updated, id');
    $this->load->view('Admin/job_post_page',$data);
  }

  public function signup() {
    $this->load->view('site/homeowner_signup');
  }
	...
	public function signup_homeowner($id) {
		...
		if($this->form_validation->run()==false){
			...
			$insert['email'] = $this->input->post('email');
			$insert['password']=$this->input->post('password');		
			$insert['county'] = $this->input->post('county_id');
			...
			if($run){
				...
				<p style="margin:0;padding:10px 0px">Thanks for registering with Tradespeople Hub! To finish your sign up, please verify your email address by clicking the button below.</p>
				<div style="text-align:center"><a href="'.site_url().'email-verified/'.$run.'/'.$insert['u_token'].'" style="background-color:#fe8a0f;color:#fff;padding:8px 22px;text-align:center;display:inline-block;line-height:25px;border-radius:3px;font-size:17px;text-decoration:none">Verify Email Address</a></div>';
		
				$json['sf'] = $this->Common_model->send_mail($this->session->userdata('email'),$subject,$contant,null,null,'support');
  ...
  public function send_verification_email($to, $subject, $from = false, $token){
		
    $user_id = $this->session->userdata('user_id');

    $html = '<p style="margin:0;font-size:20px;padding-bottom:5px;color:#2875d7">Please verify your email address.</p>
		<p style="margin:0;padding:10px 0px">Thanks for registering with Tradespeople Hub! To finish your sign up, please verify your email address by clicking the button below.</p>
		<div style="text-align:center"><a href="'.site_url().'email-verified/'.$user_id.'/'.$token.'" style="background-color:#fe8a0f;color:#fff;padding:8px 22px;text-align:center;display:inline-block;line-height:25px;border-radius:3px;font-size:17px;text-decoration:none">Verify Email Address</a></div>';
		
		return $sent = $this->Common_model->send_mail($to,$subject,$html,null,null,'support');
  ...
  private function send_how_it_works_email($to, $username, $subject){
    ...
    return $this->Common_model->send_mail($to, $subject, $html,null,null,'support');
  }
