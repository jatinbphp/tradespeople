...
$trades_man=array();
$gettradesman=$mysqli->prepare("select id,f_name,l_name,profile,trading_name,city,county,about_business,cdate,category from users where type = 1 and status = 0 and u_email_verify = 1 order by (select tr_id from rating_table where rating_table.rt_rateTo = users.id order by tr_id desc limit 1) desc");
