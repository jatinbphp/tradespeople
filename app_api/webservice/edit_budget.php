...
if($check_user <= 0){
	$record		=	array('success'=>'false1', 'msg'=>array('Something went wrong, please try again later')); 
	jsonSendEncode($record);
}
...
if($get_bid_record1 <= 0){
	$record		=	array('success'=>'false2',  'msg'=>array('Something went wrong, please try again later')); 
	jsonSendEncode($record);
}
...
if($get_job_record11 <= 0){
	$record		=	array('success'=>'false3',  'msg'=>array('Something went wrong, please try again later')); 
	jsonSendEncode($record);
}
...
$update_job = $mysqli->prepare("UPDATE `tbl_jobs` SET `budget`=?  WHERE  job_id=?");
...
$update_job1 = $mysqli->prepare("UPDATE `tbl_jobpost_bids` SET `bid_amount`=? , delivery_days=
?  WHERE  id=?");
...
$record=array('success'=>'true','msg'=>array('Job budget has been updated successfully')); 
jsonSendEncode($record);
