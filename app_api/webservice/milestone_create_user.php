...
// ======================================
$bid_arr = array();
$get_bid_details=$mysqli->prepare("SELECT id,posted_by,bid_by,total_milestone_amount,bid_amount,propose_description,delivery_days,bid_by,status FROM tbl_jobpost_bids WHERE  id= ? and (status = 7 or status = 3 or status = 5 or status = 10 or status = 4)");
...
if(count($bid_arr)>0 && count($job_arr)>0){
		...
		if($milestone_amount > $u_wallet) {
			...
			$transaction_add->bind_param("isiisssssss",$user_id,$milestone_amount,$tr_type,$tr_status,$updatetime,$updatetime,$tr_message,$transactionid,$action,$action_id,$action_json);	
