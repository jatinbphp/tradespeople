...
$check_user_all	=	$mysqli->prepare("SELECT id,withdrawable_balance,spend_amount,f_name,l_name,email from users where id=?");
...
if($amount > $u_wallet){
	...
	if($amount > $get_commision['p_max_w'] || $amount < $get_commision['p_min_w']){
		...
		$tr_message='You have withdrawn £'.$amount.' from your wallet.';
